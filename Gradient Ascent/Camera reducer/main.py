...
def main_calculate(file_path, effective_range, grid_distance, crime_type, cctv_number, cctv_effectivity, avg_damage, gradient_area):
    ...
    crime_matrix_test_final_crime, crime_matrix_test_cctv = cctv_placement(crime_matrix_test, cctv_number, grid_distance, effective_range, cctv_effectivity, gradient_area)
    # Time messurement
    end_time = time.time()
...
entry_8 = customtkinter.CTkEntry(master=frame, placeholder_text="Gradientenreichweite", height=35, width=500)
entry_8.pack(pady=12, padx=10)

button = customtkinter.CTkButton(master=frame, text="Positionen berechnen", command= lambda: main_calculate(entry_1.get(), float(entry_2.get()), float(entry_3.get()), entry_4.get(), float(entry_5.get()), float(entry_6.get()), float(entry_7.get()), int(entry_8.get())), height=35, width=500)
