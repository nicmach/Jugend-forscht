...
def calculate_difference_matrix(crime_matrix, coordinates_changed, distance=0.5, cctv_effectivity=0.078, effectivity=0.2):
...
def calculate_difference(cur_row, cur_col, crime_matrix, final_difference_crime_matrix, choords_checked, coordinates_changed, number_of_fields, effectivity):
  if not((cur_row,cur_col) in choords_checked):
    sum = 0
    num_rows, num_cols = crime_matrix.shape
        
    if coordinates_changed[cur_row][cur_col] != 1:
      sum = crime_matrix[cur_row][cur_col]
    ...
    for k in range(1, number_of_fields+1):
      ...
      if (row_ab < num_rows) and (row_ab > cur_row) and (coordinates_changed[row_ab][cur_col] != 1):  
            sum += crime_matrix[row_ab][cur_col]
      if (row_un >= 0) and (row_un < cur_row) and (coordinates_changed[row_un][cur_col] != 1):  
        sum += crime_matrix[row_un][cur_col]
      if (col_ri < num_cols) and (col_ri > cur_col) and (coordinates_changed[cur_row][col_ri] != 1): 
        sum += crime_matrix[cur_row][col_ri]
      if (col_le >= 0) and (col_le < cur_col) and (coordinates_changed[cur_row][col_le] != 1):   
        sum += crime_matrix[cur_row][col_le]

      final_difference_crime_matrix[cur_row][cur_col] = sum * effectivity
...
def update_difference_matrix(crime_matrix, crime_difference_matrix, coordinates_changed, coordinates_last_changed, distance, cctv_effectivity=0.078, effectivity=0.2):
  ...
  for coord in coordinates_last_changed:
    ...
    for k in range(1, number_of_fields+1):
      ...
      calculate_difference(row_above, current_col, crime_matrix, crime_difference_matrix, choords_checked, coordinates_changed, number_of_fields, effectivity)
      calculate_difference(row_under, current_col, crime_matrix, crime_difference_matrix, choords_checked, coordinates_changed, number_of_fields, effectivity)
      calculate_difference(current_row, col_right, crime_matrix, crime_difference_matrix, choords_checked, coordinates_changed, number_of_fields, effectivity)
      calculate_difference(current_row, col_left, crime_matrix, crime_difference_matrix, choords_checked, coordinates_changed, number_of_fields, effectivity)
  return crime_difference_matrix
...
def cctv_placement(crime_matrix, cctv_number, distance=0.05, cctv_effectivity=0.078, effectivity=0.2):
  ...
  while i < cctv_number:
    ...
    if (cctv_matrix[current_row, current_column] != 2) and (cctv_matrix[current_row, current_column] != 1): 
      ...
      coordinates_last_changed.append((current_row, current_column))
      final_crime_matrix[current_row, current_column] = final_crime_matrix[current_row, current_column] * (1 - effectivity)
      final_difference_crime_matrix[current_row, current_column] = 0
    ...
    for k in range(1, number_of_fields+1):
        ...
        if (row_above < num_rows) and (row_above > current_row) and (cctv_matrix[row_above, current_column] != 1) and (cctv_matrix[row_above, current_column] != 2): 
          ...
          final_difference_crime_matrix[row_above, current_column] = 0

          coordinates_changed[row_above, current_column] = 1
          coordinates_last_changed.append((row_above, current_column))
        ...
        if (row_under >= 0) and (row_under < current_row) and (cctv_matrix[row_under, current_column] != 1) and (cctv_matrix[row_under, current_column] != 2): 
          ...
          final_difference_crime_matrix[row_under, current_column] = 0

          coordinates_changed[row_under, current_column] = 1
          coordinates_last_changed.append((row_under, current_column))
        ...
        if (col_right < num_cols) and (col_right > current_column) and (cctv_matrix[current_row, col_right] != 1) and (cctv_matrix[current_row, col_right] != 2):
          ...
          final_difference_crime_matrix[current_row, col_right] = 0

          coordinates_changed[current_row, col_right] = 1
          coordinates_last_changed.append((current_row, col_right))
        ...
        if (col_left >= 0) and (col_left < current_column) and (cctv_matrix[current_row, col_left] != 1) and (cctv_matrix[current_row, col_left] != 2):   
          ...
          final_difference_crime_matrix[current_row, col_left] = 0


          coordinates_changed[current_row, col_left] = 1
          coordinates_last_changed.append((current_row, col_left))
